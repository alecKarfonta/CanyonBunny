...
import com.libgdxbook.canyonbunny.models.BunnyHead.JUMP_STATE;
...
import com.libgdxbook.canyonbunny.views.Level;
import com.libgdxbook.canyonbunny.views.MenuScreen;
import com.libgdxbook.canyonbunny.views.transitions.ScreenTransition;
import com.libgdxbook.canyonbunny.views.transitions.ScreenTransitionSlide;
...
public class WorldController extends InputAdapter implements Disposable {
	...
	public float livesVisual, scoreVisual;
	...
	private void initPhysics() {
		// if the world already exists, dispose of it
		if (world != null)
			world.dispose();
		...
		// create box2d bodies for all the rocks
		for (Ground rock : level.rocks) {
			BodyDef bodyDef = new BodyDef();
			bodyDef.type = BodyType.KinematicBody; // kinematic because they
													// float up and down
	...
	private void initLevel() {
		...
		initPhysics(); // must call initPhysics after level is built because it
						// depends on the rocks in the level
	}
	...
	public void update(float deltaTime) {
		...
		// if the game is over, or the goal is reached
		if (isGameOver() || goalReached) {
			// start the time left after game over timer
			timeLeftGameOverDelay -= deltaTime;
			// when the time left after game over time is done
		...
		// if the game is not over and the player dies
		if (!isGameOver() && isPlayerInWater()) {
			// play lives lost sound clip
		...
		// if the visual score is less than the score set the score to
		// the min of the actual score and the current scoreVisual plus
		// a value proportional to deltaTime
	...
	private void backToMenu() {
		// slide the menu screen in from the top
		ScreenTransition transition = 
				ScreenTransitionSlide.init(0.75f, ScreenTransitionSlide.DOWN, false, Interpolation.bounceOut);
