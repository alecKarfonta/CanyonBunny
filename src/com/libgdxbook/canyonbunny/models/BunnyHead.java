...
public class BunnyHead extends Entity {
	...
	public void update(float deltaTime) {
		...
		// if the player has a feather power up
		if (timeLeftFeatherPowerup > 0) {
			// if the animation is at the copter phase
			...
			// if the feather power up runs out
			if (timeLeftFeatherPowerup < 0) {
		...
		// if the player has feather power up
		if (hasFeatherPowerup) {
			// if the animation is currently normal
			if (animation == animNormal) {
				// start the copter animation
				setAnimation(animCopterTransform);
			// else if the animation is currently copter
			} else if (animation == animCopterTransform) {
				// if the last animation is finished
				if (animation.isAnimationFinished(stateTime))
					// rotate one more time?
					setAnimation(animCopterRotate);
			}
		// else the player does not have feather power up	
		} else {
			// if the animation is currently rotate
			if (animation == animCopterRotate) {
				// if the last animation is finished
				if (animation.isAnimationFinished(stateTime))
					// start the transform back animation
					setAnimation(animCopterTransformBack);
			// if the current animation is transform back	
			} else if (animation == animCopterTransformBack) {
				// if the animaion is finished
				if (animation.isAnimationFinished(stateTime))
	...
	public void setJumping(boolean jumpKeyPressed) {
		...
		case JUMP_FALLING: // Falling down after jump
			// if the player is in the air and has a feather, she can use it to
			// jump
